...
def PCA_LFI_calculator(df_pre, df_post, cov_pre, cov_post):
    ...
    corr_est = cov_post / (sigma.reshape(-1, 1) @ sigma.reshape(1, -1))
    ...
    assert_allclose(df_abs_pre ** 2 / (N * lambda_sum_pre) *
                    (unit_df_pre.reshape(1, -1) @
                     eig_vec_pre @ np.diag(1 / unit_eig_val_pre) @ eig_vec_pre.T
                     @ unit_df_pre.reshape(-1, 1)),
                    LFI_calculator(df_pre, cov_pre), rtol=1e-02)
    assert_allclose(df_abs_post ** 2 / (N * lambda_sum_post) *
                    (unit_df_post.reshape(1, -1) @
                     eig_vec_post @ np.diag(1 / unit_eig_val_post) @ eig_vec_post.T
                     @ unit_df_post.reshape(-1, 1)),
                    LFI_calculator(df_post, cov_post), rtol=1e-02)
    ...
    return temp, eig_pairs_pre, eig_pairs_post
