...
from model.define_test_loop import test_loop

from datetime import datetime


def pretest(ref_direction, target_sep, runs, repetition, nepoch, coherence_levels):
    ...
    if nepoch is None:
        ...
        model.load_state_dict(torch.load(weightspath + "/Pre-train_"+str(nepoch)+"_model_weights.pt", map_location=device)['model_state_dict'])
    else:
    ...
    if nepoch is None:
        np.save(weightspath + '/ACC_pre-test.npy', ACC)
        np.save(weightspath + '/Loss_pre-test.npy', Test_Loss)
        with open(weightspath + "/FiringRate_Pre.pkl", 'wb') as f:
            pickle.dump({'FiringRate_X': FiringRate_X, 'FiringRate_Y': FiringRate_Y}, f, pickle.HIGHEST_PROTOCOL)
    elif isinstance(nepoch, int):
        np.save(weightspath + "/ACC_pre-train_"+str(nepoch)+".npy", ACC)
        np.save(weightspath + "/Loss_pre-train_" + str(nepoch) + ".npy", Test_Loss)
        with open(weightspath + "/FiringRate_Pre-train_"+str(nepoch)+".pkl", 'wb') as f:
            pickle.dump({'FiringRate_X': FiringRate_X, 'FiringRate_Y': FiringRate_Y}, f, pickle.HIGHEST_PROTOCOL)
    else:
        ValueError("Wrong nepoch value!")
    print("Done!")
